...
contract MultiEnglishAuction is AccessControl
{
    ...
    event PushWithdraw(uint auctionId, address addr, uint bal);
    ...
    /**
    Ability for  admin role to push a withdrawl
     */
    function pushWithdrawl(uint _auctionId, address _addr)
        public
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        // ensure auction is in ENDED state
        require(auctions[_auctionId].auctionStatus == AuctionStatus.ENDED, "Auction state invalid (required ENDED)");

        // ensure withdrwal is not for highest bidder
        require(_addr != auctions[_auctionId].highestBidder, "Highest bidder can't withdraw");

        // ensure bidder has a withdrawl balance
        require(bids[_auctionId][_addr] > 0, "No pending bids");

        uint bal = bids[_auctionId][_addr];
        bids[_auctionId][_addr] = 0;
        payable(_addr).transfer(bal);

        emit PushWithdraw(_auctionId, _addr, bal);
    }
